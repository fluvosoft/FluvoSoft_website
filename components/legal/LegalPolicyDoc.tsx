...
import type { PrivacyDocArticle, PrivacyDocChapter } from "@/data/fluvoSoftPrivacy";
...
function ArticleBody({ paragraphs, bullets, links }: PrivacyDocArticle) {
  return (
...
type LegalPolicyDocProps = {
  chapters: PrivacyDocChapter[];
  articleIds: string[];
  heading: string;
  intro: string;
  navLabel: string;
...
export default function LegalPolicyDoc({
  chapters,
  articleIds,
  heading,
  intro,
  ...
  const [activeId, setActiveId] = useState<string>(articleIds[0] ?? "");
  ...
  return (
    <div className="mx-auto max-w-container">
      <div className="max-w-3xl">
        <h2 className="text-2xl font-medium text-foreground md:text-3xl">{heading}</h2>
        <p className="mt-4 text-accent">{intro}</p>
      </div>
