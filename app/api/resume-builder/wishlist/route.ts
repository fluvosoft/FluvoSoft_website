...
export async function POST(req: NextRequest) {
  try {
    const ip = getIp(req);
    if (!checkRateLimit(`resume-builder-wishlist:${ip}`, 5, 60_000)) {
    ...
    const payload = {
      ...
      source: "resume-builder-wishlist",
      ipHashHint: ip.slice(0, 6),
      createdAt: serverTimestamp(),
    };

    // set() retries forever if the RTDB URL is wrong/unreachable — fail fast instead
    await Promise.race([
      set(ref(rtdb, `resumeBuilderWishlist/${id}`), payload),
    ...
    console.error("Resume Builder wishlist write failed:", error);
    return NextResponse.json(
